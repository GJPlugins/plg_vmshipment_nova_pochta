	...
	class plgVmShipmentNova_pochta extends vmPSPlugin
	{
		...
		function __construct ( &$subject , $config )
		{
			...
			if( $app->isAdmin() && $this->_name == _SHIP_NAME && $option == 'com_virtuemart' && $view == 'shipmentmethod' && $task == 'edit' )
			{
				...
				if( !class_exists( 'addJsShipmentMethodEdit' ) ) require( JPATH_PLUGINS . DS . 'vmshipment' . DS . $this->_name . DS . $this->_name . DS . 'helpers' . DS . 'shipmentMethodEdit.php' );
		...
		{
			$SQLfields = [ 'id' => 'int(1) UNSIGNED NOT NULL AUTO_INCREMENT' , 'user_id' => 'int(1) UNSIGNED NOT NULL' , 'virtuemart_order_id' => 'int(11) UNSIGNED' , 'order_number' => 'char(32)' , 'virtuemart_shipmentmethod_id' => 'mediumint(1) UNSIGNED' ,
			
			
			               'FirstName' => 'char(18)' , 'LastName' => 'varchar(18)' , 'MiddleName' => 'char(18)' , // 'Phone'      => 'varchar(18)   ',
				...
				// min - 0,1 Вес фактический, кг
				           'Weight'                  => 'decimal(5,2)' , # Объем общий, м.куб (min - 0.0004), обязательно для заполнения, если не указаны значения OptionsSeat
				           'VolumeWeight'            => 'decimal(7,4)' , # Общий объем м³
				           'VolumeGeneral'           => 'decimal(12,8)' , # количество мест отправления
				           'SeatsAmount'             => 'int(1) NOT NULL' , # Параметры репет табле
				...
				// Текстовое поле, вводиться для доп. описания
				           'Description'             => 'varchar(50)' , // Целое число, объявленная стоимость
				// (если объявленная стоимость не указана,
				// API автоматически подставит минимальную объявленную
				// цену - 300.00
				           'Cost'                    => 'int(1)' , // Идентификатор города получателя
				           'CityRecipient'           => 'char(36)' , // Идентификатор получателя
				           'Recipient'               => 'char(36)' , // Идентификатор адреса получателя
				           'RecipientAddress'        => 'char(36)' , // Идентификатор контактного лица получателя
				...
				# Ref некладной
				           'IntDocNumber'            => 'char(36)' , #Прогноз даты доставки:
				...
				// 'date_otpravki'        => 'date NOT NULL DEFAULT \'0000-00-00\'',
				           ...
				           'shipment_weight_unit' => 'char(3) DEFAULT \'KG\'' , /*'col_mest'             => 'varchar(2)',*/
				           'shipment_cost'        => 'decimal(10,2)' , 'shipment_package_fee' => 'decimal(10,2)' , 'tax_id' => 'smallint(1)' , ];
		...
		function getHtmlNameVarianDelivery ( $shipment_name , $citi_of_sender , $warenhouseClient )
		{
			...
			if( !class_exists( 'np_param' ) ) require( JPATH_ROOT . DS . 'plugins' . DS . 'vmshipment' . DS . 'nova_pochta' . DS . 'nova_pochta' . DS . 'np_param.php' );
		...
		function plgVmConfirmedOrder ( VirtueMartCart $cart , $order )
		{
			...
			# Подключить логер
			if( !class_exists( 'shipmentMethodLog' ) )
				require( JPATH_PLUGINS . DS . 'vmshipment' . DS . $this->_name . DS . $this->_name . DS . 'helpers' . DS . 'shipmentMethodLog.php' );
			...
			if( !class_exists( 'helpersShipmentMethodCart' ) ) require( JPATH_PLUGINS . DS . 'vmshipment' . DS . $this->_name . DS . $this->_name . DS . 'helpers' . DS . 'shipmentMethodCart.php' );
			...
			/*echo'<pre>';print_r( $CounterpartyData );echo'</pre>'.__FILE__.' '.__LINE__;
		...
		{
			$db = JFactory::getDBO();
			$q  = 'SELECT * , DATE_FORMAT(DateTime,"%d.%m.%Y") AS DateTime FROM `' . $this->_tablename . '` ' . 'WHERE `virtuemart_order_id` = ' . $virtuemart_order_id;
			$db->setQuery( $q );
			...
			if( !class_exists( 'helpersShipmentMethodOrderEdit' ) ) require( JPATH_PLUGINS . DS . 'vmshipment' . DS . $this->_name . DS . $this->_name . DS . 'helpers' . DS . 'shipmentMethodOrderEdit.php' );
		...
		private function _nbproductsCond ( $cart , $method )
		{
			...
			if( $nbproducts )
			{
				$nbproducts_cond = ( $nbproducts >= $method->nbproducts_start AND $nbproducts <= $method->nbproducts_stop OR ( $method->nbproducts_start <= $nbproducts AND ( $method->nbproducts_stop == 0 ) ) );
			}
			else
			{
		...
		private function _orderamountCond ( $cart_prices , $method )
		{
			...
			{
				$orderamount_cond = ( $cart_prices[ 'salesPrice' ] >= $method->orderamount_start AND $cart_prices[ 'salesPrice' ] <= $method->orderamount_stop OR ( $method->orderamount_start <= $cart_prices[ 'salesPrice' ] AND ( $method->orderamount_stop == 0 ) ) );
			}
			else
			{
		...
		public function plgVmOnSelectCheckShipment ( VirtueMartCart &$cart )
		{
			...
			$session->set( _SHIP_NAME , [ 'cartData' => $dataShipment ] );
		...
		protected function getPluginHtml ( $plugin , $selectedPlugin , $pluginSalesPrice )
		{
			...
			$_plgNovaPochta = $app->input->getArray( [ '_plgNovaPochta' => [ 'automaticSelectedShipment' => 'BOOL' ] ] );
		 ...
		 *
		 * @return bool|null if the method was not selected, false if the shiiping rate is not valid any more, true
		 *                   otherwise
		 * @since     3.8
		 * @author    Gartes
		 * @copyright 03.11.18
		...
		function plgVmOnCheckAutomaticSelectedShipment ( VirtueMartCart $cart , array $cart_prices = [] , &$shipCounter )
		{
			...
			if( !$shipCounter )
			{
				...
				$_plgNovaPochta                                                    = $app->input->getArray( [ '_plgNovaPochta' => 'ARRAY' ] );
		 ...
		 * @return boolean True when the data was valid, false otherwise. If the plugin is not activated, it should
		 *                 return null.
		 */
		 ...
		 * Это может быть использован для отображения линии специальных кодов пакета, например, со ссылкой на внешних
		 * отслеживания и Системы отслеживания
		 *
		 * @param   integer  $_orderId  The order ID
		 * @param   integer  $_lineId
		 *
